...
export function Navbar() {
  ...
  return (
    <motion.header 
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled || mobileMenuOpen ? 'glass border-b border-white/5 py-4' : 'bg-transparent py-6'}`}
    >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 right-0 bg-primary/95 backdrop-blur-xl border-b border-white/5 h-screen md:hidden flex flex-col pt-8 pb-24 px-6 overflow-y-auto"
          >
            <nav className="flex flex-col gap-6">
              {navLinks.map((link) => (
                <Link 
                  key={link.name} 
                  href={link.path}
                  className={`text-2xl font-serif transition-colors ${pathname === link.path ? 'text-accent' : 'text-white hover:text-white/80'}`}
                >
                  {link.name}
                </Link>
              ))}
